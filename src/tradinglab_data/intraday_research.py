...
class IntradayResearchInspectEntry:
    symbol: str
    exists: bool
    rows: int
    start: str | None
...
def intraday_research_parquet_path(root: str | Path, *, interval: str, symbol: str) -> Path:
    return Path(root) / interval / f"{symbol}.parquet"


def empty_intraday_research_frame() -> pl.DataFrame:
    return pl.DataFrame(schema=INTRADAY_RESEARCH_PARQUET_SCHEMA)
...
def _validate_options(*, interval: str, provider: str, session: str) -> None:
    if interval not in SUPPORTED_INTERVALS:
        supported = ", ".join(sorted(SUPPORTED_INTERVALS))
        raise ValueError(f"Unsupported intraday research interval: {interval!r}. Supported intervals: {supported}.")
    if provider.strip().lower() != SUPPORTED_PROVIDER:
        raise ValueError(f"Unsupported intraday research provider: {provider!r}. Supported provider: {SUPPORTED_PROVIDER}.")
    if session.strip().lower() != SUPPORTED_SESSION:
...
def normalize_intraday_research_frame(
    ...
    *,
    ...
    session: str = SUPPORTED_SESSION,
    exchange_timezone: str = DEFAULT_EXCHANGE_TIMEZONE,
    ingested_at: datetime | None = None,
) -> pl.DataFrame:
    _validate_options(interval=interval, provider=provider, session=session)
    ...
    out = (
        ...
        .with_columns(
            [
                ...
                pl.lit(session.strip().lower()).cast(pl.String).alias("session"),
                pl.col("timestamp")
                .dt.replace_time_zone("UTC")
                .dt.convert_time_zone(tz_name)
                .dt.date()
                .alias("session_date"),
                pl.lit(True).cast(pl.Boolean).alias("is_regular_session"),
                pl.lit(ingested_at or _utc_now_naive()).cast(pl.Datetime).alias("ingested_at"),
            ]
        )
        .unique(subset=["timestamp"], keep="last")
        .sort("timestamp")
        .select(list(INTRADAY_RESEARCH_PARQUET_SCHEMA))
    )
    return out


def trim_intraday_research_window(df: pl.DataFrame, *, retention_days: int) -> pl.DataFrame:
...
def update_intraday_research_store(
    symbols: list[str],
    *,
    ...
    session: str = SUPPORTED_SESSION,
    exchange_timezone: str = DEFAULT_EXCHANGE_TIMEZONE,
    universe_name: str = "intraday_pilot",
    ...
    existing = [symbol for symbol in symbols if intraday_research_parquet_path(root, interval=interval, symbol=symbol).exists()]
    ...
    initial_map = (
        fetch_intraday_fn(
            symbols=missing,
            interval=interval,
            period=period_for_interval(interval, MAX_PERIOD_BY_INTERVAL, purpose="initial intraday research fetch"),
            prepost=False,
    ...
    update_map = (
        fetch_intraday_fn(
            symbols=existing,
            interval=interval,
            period=period_for_interval(interval, update_period, purpose="incremental intraday research fetch"),
            prepost=False,
            chunk_size=chunk_size,
            sleep_seconds=sleep_seconds,
            max_retries=max_retries,
    ...
    for symbol in symbols:
        path = intraday_research_parquet_path(root, interval=interval, symbol=symbol)
        fetched = initial_map.get(symbol) if symbol in initial_map else update_map.get(symbol)
        existing_df = read_frame_fn(path)
        current = empty_intraday_research_frame() if existing_df is None or existing_df.is_empty() else existing_df
        currency = (fetch_currency_fn(symbol) or "UNKNOWN").strip().upper() or "UNKNOWN"
        new_rows = normalize_intraday_research_frame(
            fetched,
            symbol=symbol,
            currency=currency,
            interval=interval,
        ...
        combined = trim_intraday_research_window(combined, retention_days=retention_days)
...
def inspect_intraday_research_store(
    symbols: list[str],
    *,
    research_root: str | Path,
    interval: str = "5m",
) -> list[dict[str, object]]:
    _validate_options(interval=interval, provider=SUPPORTED_PROVIDER, session=SUPPORTED_SESSION)
    root = Path(research_root)
...
def validate_intraday_research_store(
    symbols: list[str],
    *,
    research_root: str | Path,
    interval: str = "5m",
    ...
    inspected = inspect_intraday_research_store(symbols, research_root=research_root, interval=interval)
    ...
    return {
        ...
        "interval": interval,
        "universe": universe_name,
        "root": str(Path(research_root) / interval),
